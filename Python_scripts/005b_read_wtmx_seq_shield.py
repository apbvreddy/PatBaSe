import sys
import os

from numpy import loadtxt
def split(seq):
        return list(seq)

file = open("wm2.csv")
numpy_array = loadtxt(file, delimiter=",",dtype=str)
print(len(numpy_array))
#print(numpy_array)
print(numpy_array[20,20])
wtmx={}
for j in range(21):
    if j==0:
        continue
    for i in range(21):
        if i==0:
            continue
        #print(numpy_array[j,0],numpy_array[0,i])
        key=numpy_array[j,0]+numpy_array[0,i]
        wtmx[key]=numpy_array[j,i]
        #print(key,"=",wtmx[key])

file2 = open(sys.argv[1])
SNO=sys.argv[2]
numpy_seq = loadtxt(file2, delimiter=",",dtype=str)
NoSeq=len(numpy_seq)
print(NoSeq)

for i in range(NoSeq):       
    seq1=split(numpy_seq[i,0])
    ocf=open(numpy_seq[i,1]+".txt",'w')
    for j in range(NoSeq):
        score=0
        #print(numpy_seq[j,0],j)
        seq2=split(numpy_seq[j,0])
        for k in range(46):
            #print(seq1[k],seq2[k])
            key=seq1[k]+seq2[k]
            score += int(wtmx[key])
        #print(numpy_seq[j,0],j,score,numpy_seq[j,1])
        pscore=round((score/920)*100.0,2)
        if pscore < 70.00:
                continue
        print(numpy_seq[j,0],j,score,pscore,numpy_seq[j,1],file = ocf)
    ocf.close()
    I=str(i)
    cmd = "sort -n -r -t' ' -k3 "+numpy_seq[i,1]+".txt > "+numpy_seq[i,1]+"_cluster_"+SNO+".txt"
    print(cmd)
    os.system(cmd)
    cmd = "rm "+numpy_seq[i,1]+".txt"
    print(cmd)
    os.system(cmd)
    
    #cmd ="dos2unix auto_"+I+"_compare.txt

    cmd = "cut -d' ' -f3,5 "+numpy_seq[i,1]+"_cluster_"+SNO+".txt | sort -n -r -t' ' -k 1| cut -d' ' -f2 > t1.del" 
    print(cmd)
    os.system(cmd)

    cmd = "grep -v -f t1.del  "+sys.argv[1]+" > "+I+"_"+sys.argv[1]
    print(cmd)
    os.system(cmd)
    
    '''
    cmd = "sed 's/^/grep /' t1.del | sed 's/$/ "+sys.argv[2]+" >> "+numpy_seq[i,1]+"cID_.csv/' > t"+I+".sh"
    print(cmd)
    os.system(cmd)
    
    cmd ="dos2unix t"+I+".del"
    print(cmd)
    os.system(cmd)    
    #cmd = "grep -f auto_"+I+"_compare.txt cIDg01to73_ampdRn_seq2.csv > autoplot_"+I+"_compare.csv"
    cmd="grep -f t"+I+".del "+sys.argv[2]+" > autoplot_"+I+"_compare.csv"
    print(cmd)
    os.system(cmd)

    cmd="grep -f t"+I+".del "+numpy_seq[i,1]+".txt | sort -n -r -t' ' -k3 > scoring_"+I+"_compare.csv"
    print(cmd)
    os.system(cmd)
      
    cmd1 = "rm "+numpy_seq[i,1]+".txt t"+I+".del"
    print(cmd1)
    os.system(cmd1)
    '''

    if i==0:
        exit()
